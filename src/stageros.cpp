...
private:
    ...
    struct StageRobot
    {
        ...
        ros::Subscriber pose_sub; //one robot_pose subscriber
    };
...
public:
    ...
    void poseReceived(int idx, const boost::shared_ptr<geometry_msgs::PoseWithCovarianceStamped const>& msg);
...
void
StageNode::poseReceived(int idx, const boost::shared_ptr<geometry_msgs::PoseWithCovarianceStamped const>& msg)
{
    boost::mutex::scoped_lock lock(msg_lock);
    tf::Quaternion q(
    ...
    Stg::Pose pose(msg->pose.pose.position.x, msg->pose.pose.position.y, 0.0, yaw);
...
StageNode::SubscribeModels()
{
	...
	// TODO - print the topic names nicely as well
        ...
        new_robot->pose_sub = n_.subscribe<geometry_msgs::PoseWithCovarianceStamped>(mapName(SET_POSE, r, static_cast<Stg::Model*>(new_robot->positionmodel)), 10, boost::bind(&StageNode::poseReceived, this, r, _1));
